...
export function NilScoreCard({ score, label, themeAccent, onRefresh, loading, hasRealData = true }: Props) {
    ...
    <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[#111113] p-6 flex flex-col items-center justify-between min-h-[300px]">
      ...
      <div className="w-full text-center">
        <div className="inline-flex items-center justify-center px-3 py-1 rounded-full border text-xs font-black tracking-wider uppercase mb-2 bg-[#16161A]/80 border-white/[0.08]" style={{ color: themeAccent }}>
          {hasRealData ? `${label} Rank` : "Baseline Assessment"}
        </div>
        <p className="text-[11px] text-white/40 leading-normal max-w-[220px] mx-auto">
          {hasRealData
            ? "Based on your verified social followers, card engagement, and profile context."
            : "Based on sport and school context only. Connect a social account to verify your reach."}
        </p>
      </div>
    </div>
  );
