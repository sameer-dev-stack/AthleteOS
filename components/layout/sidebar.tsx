  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-50 h-10 w-10 rounded-lg bg-[#16161A] border border-white/[0.08] flex items-center justify-center text-white/60 hover:text-white transition-colors"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <div className="relative w-[280px] h-full bg-[#0C0C0E] border-r border-white/[0.06] flex flex-col animate-drawer-in">
            {/* Mobile header */}
            <div className="flex h-16 items-center px-5 border-b border-white/[0.06]">
              <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-[8px] bg-accent flex items-center justify-center">
                  <span className="text-bg font-black text-[10px] tracking-widest select-none">AOS</span>
                </div>
                <span className="font-black tracking-[0.15em] uppercase text-sm text-accent">
                  ATHLETEOS
                </span>
              </Link>
            </div>
                    ...
                    {section.items.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          href={item.href}
                          onClick={() => setMobileOpen(false)}
                          className={`flex items-center gap-3 h-10 px-3 rounded-lg transition-all duration-150 ${
                            isActive
                              ? "bg-accent/10 text-accent"
                              : "text-white/45 hover:text-white hover:bg-white/[0.04]"
                          }`}
            ...
            <div className="border-t border-white/[0.06] p-3 flex-shrink-0">
              <div className="flex items-center gap-3 rounded-lg p-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/30 truncate mt-0.5 leading-none">{email}</p>
                </div>
                <form action={signOut}>
                  <button
                    type="submit"
