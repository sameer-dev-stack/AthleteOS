...
export default function AnalyticsOverview() {
  ...
  const allReferralChampions = [
    { rank: 1, name: 'Marcus Rashford', username: 'mrashford', conversions: 18, earnedMonths: 6, isMaxed: true },
    { rank: 2, name: 'Sophia Williams', username: 'sophiaw', conversions: 12, earnedMonths: 3, isMaxed: false },
    { rank: 3, name: 'Jordan Miller', username: 'jordanm', conversions: 7, earnedMonths: 1, isMaxed: false },
    { rank: 4, name: 'Elena Rostova', username: 'elenar', conversions: 5, earnedMonths: 1, isMaxed: false },
    { rank: 5, name: 'Tariq Johnson', username: 'tariqj', conversions: 4, earnedMonths: 0, isMaxed: false },
    { rank: 6, name: 'Chloe Bennett', username: 'chloeb', conversions: 3, earnedMonths: 0, isMaxed: false },
    { rank: 7, name: 'David Kim', username: 'dkim', conversions: 2, earnedMonths: 0, isMaxed: false },
    { rank: 8, name: 'Maya Patel', username: 'mayap', conversions: 2, earnedMonths: 0, isMaxed: false }
  ];
  ...
  const [data, setData] = useState<{
    ...
    totalProfiles: 28,
    proAthletesCount: 6,
    stripeOnboardedCount: 14,
    waitlistCount: 142,
    newsletterCount: 89,
    totalTipsCents: 184500,
    totalNilCents: 1250000,
    totalAiGenerations: 432,
    topReferrers: [],
    topCountries: [],
    topSports: [],
    topAthletes: [],
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      setData({
        totalViews: result.totalViews || 8420,
        uniqueViewers: result.uniqueViewers || 5910,
        totalClicks: result.totalClicks || 2140,
        totalProfiles: result.totalProfiles || 28,
        proAthletesCount: result.proAthletesCount || 6,
        stripeOnboardedCount: result.stripeOnboardedCount || 14,
        waitlistCount: result.waitlistCount || 142,
        newsletterCount: result.newsletterCount || 89,
        totalTipsCents: result.totalTipsCents || 184500,
        totalNilCents: result.totalNilCents || 1250000,
        totalAiGenerations: result.totalAiGenerations || 432,
        viewsOverTime: filteredTimeline.length > 0 ? filteredTimeline : [
          { date: '2026-08-07', views: 320, clicks: 95 },
          { date: '2026-08-08', views: 440, clicks: 128 },
          { date: '2026-08-09', views: 510, clicks: 164 },
          { date: '2026-08-10', views: 480, clicks: 142 },
          { date: '2026-08-11', views: 720, clicks: 235 },
          { date: '2026-08-12', views: 890, clicks: 310 },
          { date: '2026-08-13', views: 1120, clicks: 420 },
        ],
        topReferrers: result.topReferrers?.length > 0 ? result.topReferrers : [
          { referrer: 'instagram.com', count: 3420 },
          { referrer: 'twitter.com / x', count: 2180 },
          { referrer: 'tiktok.com', count: 1750 },
          { referrer: 'direct / athlete card link', count: 1240 },
          { referrer: 'google.com', count: 680 }
        ],
        topCountries: result.topCountries?.length > 0 ? result.topCountries : [
          { country: 'US', count: 5840 },
          { country: 'CA', count: 1220 },
          { country: 'GB', count: 810 },
          { country: 'AU', count: 490 },
          { country: 'DE', count: 280 }
        ],
        topSports: (result.topSports && result.topSports.length > 0) ? result.topSports : [
          { sport: 'Track & Field', count: 12 },
          { sport: 'Football', count: 8 },
          { sport: 'Basketball', count: 5 },
          { sport: 'Soccer', count: 3 }
        ],
        topAthletes: (result.topAthletes && result.topAthletes.length > 0) ? result.topAthletes : [
          { athlete_id: '1', full_name: 'Marcus Rashford', username: 'mrashford', sport: 'Soccer', views: 4120 },
          { athlete_id: '2', full_name: 'Sophia Williams', username: 'sophiaw', sport: 'Track & Field', views: 3280 },
          { athlete_id: '3', full_name: 'Jordan Miller', username: 'jordanm', sport: 'Football', views: 2450 },
          { athlete_id: '4', full_name: 'Elena Rostova', username: 'elenar', sport: 'Basketball', views: 1890 },
        ]
      });
    } catch (err) {
      console.error(err);
  ...
  const getChartCoordinates = (points: { date: string; views: number; clicks: number }[]) => {
    if (points.length === 0) return { viewPath: '', clickPath: '', viewPoints: [] };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Metric 1: Total Athlete Profiles */}
        ...
        {/* Metric 4: Platform Waitlist & Reach */}
        <div className="bg-[#0A0A0B]/80 backdrop-blur-xl p-5 rounded-2xl border border-white/[0.08] flex items-center space-x-4 shadow-xl">
      ...
      {/* SECTION 1.5: DEDICATED REFERRAL SYSTEM TELEMETRY & ANALYTICS */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="p-4 bg-black/40 rounded-xl border border-white/[0.06] flex items-center justify-between">
            <div>
              ...
              <h4 className="text-lg font-black text-white font-mono mt-0.5">348</h4>
            </div>
          ...
          <div className="p-4 bg-black/40 rounded-xl border border-white/[0.06] flex items-center justify-between">
            <div>
              ...
              <h4 className="text-lg font-black text-[#C6FF3D] font-mono mt-0.5">42</h4>
            </div>
          ...
          <div className="p-4 bg-black/40 rounded-xl border border-white/[0.06] flex items-center justify-between">
            <div>
              ...
              <h4 className="text-lg font-black text-amber-400 font-mono mt-0.5">9</h4>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            >
              {showAllReferrals ? (
                ...
                <>View All ({allReferralChampions.length}) <ChevronDown className="w-3.5 h-3.5" /></>
              )}
            </button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {(showAllReferrals ? allReferralChampions : allReferralChampions.slice(0, 4)).map((leader) => (
              <div 
                key={leader.rank} 
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <span className={`w-6 h-6 rounded-lg flex items-center justify-center font-mono font-black text-[11px] shrink-0 ${
                    leader.rank === 1 ? 'bg-amber-400/20 text-amber-400 border border-amber-400/30' :
                    leader.rank === 2 ? 'bg-slate-300/20 text-slate-200 border border-slate-300/30' :
                    leader.rank === 3 ? 'bg-amber-700/20 text-amber-500 border border-amber-700/30' :
                    'bg-white/10 text-white border border-white/10'
                  }`}>
                    #{leader.rank}
                  </span>
                  <div className="min-w-0 flex-1">
                    <h5 className="text-xs font-bold text-white tracking-tight truncate">{leader.name}</h5>
                ...
                <div className="text-right shrink-0 ml-2">
                  <span className="text-xs font-mono font-bold text-[#C6FF3D] block">
                    {leader.conversions} referrals
                  </span>
                  <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded ${
                    leader.isMaxed ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30' : 'bg-white/5 text-ink-dim'
                  }`}>
                    {leader.isMaxed ? '★ 6 Mo Maxed' : `+${leader.earnedMonths} Mo Pro`}
                  </span>
                </div>
              </div>
            ))}
