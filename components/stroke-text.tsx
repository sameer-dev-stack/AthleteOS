...
export const StrokeText: React.FC<StrokeTextProps> = ({
  ...
  useEffect(() => {
    ...
    const strokes = gsap.utils.toArray(root.querySelectorAll('[data-stroke-char]')) as HTMLElement[];
    const fills = gsap.utils.toArray(root.querySelectorAll('[data-fill-char]')) as HTMLElement[];
    ...
    const targets = [...strokes, ...fills, wipe].filter(Boolean) as (HTMLElement | SVGRectElement)[];
