...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: athletes, error: athletesErr } = await admin
      .from("profiles")
      .select("id, email, full_name, sport, school, position, bio, created_at, plan, extended_pro_until, pro_expires_at, stripe_subscription_id")
    ...
    for (const athlete of athletes) {
      try {
        ...
        const effectivePlan = resolvePlan(athlete.plan, athlete.extended_pro_until, athlete.pro_expires_at, athlete.stripe_subscription_id);
        ...
        // Pro only — Free tier athletes do not receive weekly briefings
