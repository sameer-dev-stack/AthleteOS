...
Full-screen square (1:1) avatar crop modal built on `react-easy-crop`. Crop area is a square (`cropShape="rect"`, `aspect={1}`); the circle is a preview overlay guide only (lightweight radial-gradient mask + ring) since the card shows the photo as a full-bleed rectangular hero (`object-cover object-top`) plus a circular avatar. Drag to position, pinch or slider to zoom, Reset returns to fit. No face detection — the user positions the crop. When `previewProfile` is passed, the "Live card preview" panel renders a scaled (0.5x) static replica of the card front face — hero, circular avatar, name, sport·position, school, accent hairlines — fed by the live 256px crop so the user sees the actual card output while dragging. Falls back to a small circular "Result" thumb when no profile is provided. Emits a cropped blob bounded to 512x512 WebP; "Use as is" center-crops to 1:1 and bounds the same, disabled when the source is under 200px.
...
| `AthletePhoto` | Hero photo (52% card height) + vignette + grain + sport-aware fallback gradient |
