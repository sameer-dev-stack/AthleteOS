...
- **Props:**
  | Prop | Type | Default | Description |
  |---|---|---|---|
  | `currentUrl` | `string \| null` | — | Currently saved avatar URL |
  | `userId` | `string` | — | Owner id, used for the `avatars/{userId}/avatar.webp` storage path |
...
- **Blend layers gated on `active` [2026-08-17]:** `.rc-noise`, `.rc-sheen`, and `.rc-theme-gradient` now render only when `active` is true. Both faces pass `active` gated on `!isFlipping` from `ProfileCard`, so during the 0.35 s 3D flip the faces are flat, blend-free textures — eliminating the per-frame GPU re-raster that made the flip stutter on phones (measured `JankV3` 26 → 8, `DroppedFrameDuration` 14 → 2 via CDP trace). See ADR-061.
- **Props:**
  | Prop | Type | Default | Description |
  |---|---|---|---|
...
- **Back-face tap fix [2026-08-17]:** the back face div's own `onClick={handleFlip}` was removed — it bubbled to the parent `motion.div`'s `onClick={handleFlip}` in the same event, so `setFlipped(f=>!f)` ran twice (net no-op) and tapping the back of the card never flipped it back (confirmed on live production). Back-face taps now bubble to the parent's single handler and flip the card. See ADR-061.
- **Coarse-pointer overrides [2026-08-16]:** on touch devices a `@media (pointer: coarse)` block in `globals.css` forces inline `backdrop-filter` off inside `.flip-card-face` and shrinks the `.card-ambient-glow` blur from `60px` to `20px`. `components/reflective-card.css` has a matching coarse-pointer block.
- **Border glow sweep re-enabled [2026-08-16]:** both `BorderGlow` instances render `animated` on all devices (the `IS_COARSE_POINTER` gate was removed) and the `@media (pointer: coarse)` block in `border-glow.css` was deleted, so the masked gradient mesh sweeps/loops around the card again on mobile and desktop. Other mobile-perf fixes that don't conflict with the sweep remain: webcam skipped on touch, `backdrop-filter` off inside `.flip-card-face`, `.card-ambient-glow` blur reduced.
- **Glow layer scope + touch gating [2026-08-18]:** `BorderGlow` now renders a dedicated `.border-glow-layer` div owning `::before` / `::after` / `.edge-light`; the per-frame `--cursor-x/y/--edge-proximity` writes land on that layer (a sibling of `.border-glow-inner`), so sweep updates no longer invalidate the card-content subtree. The unused per-frame `--cursor-angle` write is removed. On `(pointer: coarse)` the sweep tick drops to ~31 fps (32 ms) and `.rc-container`'s hover box-shadow transition is gated behind `(hover: hover) and (pointer: fine)` so Android taps don't run blur transitions. See ADR-065.
- **Snake-head transform [2026-08-18]:** the rAF perimeter loop no longer writes CSS custom properties every frame. Instead it moves a dedicated `.snake-head` child element via `transform: translate3d(...)`. Because `transform` changes are compositor-only, the snake travels at 60fps with zero main-thread style recalc, restoring the old perimeter snake look without the FPS cost. `will-change: transform` promotes the snake head to its own compositor layer. On `(pointer: coarse)` the snake head is hidden and a CSS `conic-gradient` orbit provides a similar moving edge glow with zero JS cost. See ADR-066.

**Sub-components (all local, not exported):**

| Component | Responsibility |
|---|---|
| `AthletePhoto` | Hero photo + vignette + grain + sport-aware fallback gradient |
| `AthleteIdentity` | Balanced row containing name, badges, sport · position, school on the left, and QR/Share actions + NIL CARD watermark on the right |
