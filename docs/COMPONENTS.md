...
- **Mobile glow fallback [2026-08-18]:** `@media (pointer: coarse)` restored in `border-glow.css`. On touch devices the expensive masked gradient mesh + `mix-blend-mode: soft-light` / `plus-lighter` are replaced by a simplified base-gradient border plus a subtle CSS breathing animation, keeping the border colorful without the per-frame mask re-raster that caused ~4.5 fps. The rAF sweep itself is also gated on `animated !== false && !isCoarsePointer`. See ADR-066.
