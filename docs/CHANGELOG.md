...
### What changed
- **`app/critical.css` + inline `<style>` in `app/layout.tsx`**: the hand-written "critical CSS" inlined after the Tailwind stylesheet re-declared ~60 standard Tailwind utility classes (`.hidden`, `.flex`, `.grid`, `.block`, `.p-4`, `.text-4xl`, `.rounded-*`, `.animate-fade-up`, etc.). Because it appears later in source order, it silently overrode every Tailwind responsive variant — most visibly the admin sidebar's `hidden lg:flex` was stuck `display: none` at all widths (the "blank side panel"). Stripped the file down to the non-conflicting core: element resets (`*, html, body, main, a`) plus the theme identity classes `.bg-bg`, `.text-ink`, `.text-accent`. All removed classes are generated by Tailwind itself, so nothing loses its definition.
- **`playwright.prod.ts` + `e2e/user-flows.spec.ts`**: base URL updated from the dead deploy URL `athlete-os-vert.vercel.app` (Vercel `DEPLOYMENT_NOT_FOUND`) to the live canonical `https://www.nilcard.app`.
- **e2e cleanup**: removed leftover diagnostic specs from prior sessions (`tmp-diag`, `tmp-final-state`, `tmp-live-detail`, `tmp-local-verify`, `tmp-verify-fix`) and the tracked `playwright.tmp.config.js`.
- **`middleware.ts`**: pre-existing uncommitted change folded in — robots.txt and sitemap.xml added to the middleware matcher exclusions.

### Why
- User-reported: admin panel side panel is blank on desktop.
- Root cause (verified via CSSOM probe against live prod + local prod build): a second `.hidden{display:none}` in the critical CSS stylesheet comes after `@media(min-width:1024px){.lg:flex{display:flex}}`, so `display:none` wins at every viewport. Playwright showed content exists in the aside (9 buttons, all nav labels) but computed `display: none` at 1280px. Same conflict was silently breaking `sm:`/`md:`/`lg:` variants of padding, text-size, and display utilities across the site.
...
- removed: `playwright.tmp.config.js`, `e2e/tmp-diag.spec.ts`, `e2e/tmp-final-state.spec.ts`, `e2e/tmp-live-detail.spec.ts`, `e2e/tmp-local-verify.spec.ts`, `e2e/tmp-verify-fix.spec.ts`
